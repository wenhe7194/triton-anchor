...
def numpy_random(
    shape, dtype_str, rs: Optional[RandomState] = None, low=None, high=None
):
    """
    ...
    if dtype_str in int_dtypes + uint_dtypes:
        ...
        return (
            rs.normal(0, 1, shape).astype("float32").view("uint32")
...
def to_triton(
    x: np.ndarray, device, dst_type=None
) -> Union[TensorWrapper, torch.Tensor]:
    """
...
def supports_tma(byval_only=False):
    ...
    return (
        ...
        and cuda_version_tuple >= min_cuda_version
    )
...
def unwrap_tensor(
    t: Union[torch.Tensor, triton.runtime.jit.TensorWrapper],
) -> torch.Tensor:
...
def _fresh_knobs_impl(skipped_attr: Optional[Set[str]] = None):
    ...
    knobs_map = {
        ...
        and knobset != knobs.base_knobs
        and name not in skipped_attr
    }
