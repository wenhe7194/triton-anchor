...
class ExternLibrary(ABC):
    ...
    def generate_stub_file(self, output_dir) -> None:
        ...
        with open(output_file, "w") as f:
            ...
            if self._format:
                subprocess.Popen(
                    ["autopep8", "-a", "-r", "-i", output_file], stdout=subprocess.PIPE
                ).communicate()
                subprocess.Popen(
                    ["isort", output_file], stdout=subprocess.PIPE
                ).communicate()
...
class Libdevice(ExternLibrary):
    ...
    def parse_symbols(self, input_file) -> None:
        if len(self.symbols) > 0:
            return
        output = (
            subprocess.check_output(["grep", "define", input_file])
            .decode()
            .splitlines()
        )
        for line in output:
    ...
    def _output_stubs(self) -> str:
        ...
        for symbols in self._symbol_groups.values():
            ...
            return_str = (
                f'\treturn core.extern_elementwise("{self._name}", libdevice_path(), ['
            )
            for arg_name in symbols[0].arg_names:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--llvm-dis", dest="llvm_dis_path", help="Path to llvm-dis", default="llvm-dis"
    )
    parser.add_argument(
        "--lib-path", dest="lib_path", help="Path to the extern library"
    )
    parser.add_argument(
        "--lib-name", dest="lib_name", help="Name of the extern library"
    )
    parser.add_argument(
        "--output", dest="output_dir", help="Output file path", default="/tmp/"
    )
