...
class TritonSemantic(Generic[TensorTy]):
    ...
    def check_ptr_type_impl(
        self, type_a: tl.dtype, type_b: tl.dtype, allow_ptr_a: bool
    ) -> None:
        if type_a.is_ptr():
            if not allow_ptr_a:
    ...
    def binary_op_type_checking_impl(
        ...
        if arithmetic_check and not lhs_sca_ty.is_ptr() and not rhs_sca_ty.is_ptr():
            ret_sca_ty = self.computation_type_impl(
                lhs_sca_ty, lhs_is_scalar, rhs_sca_ty, rhs_is_scalar, div_or_mod
            )
            if (
                lhs_is_scalar
                and lhs_scalar < 0
                ...
                raise ValueError(
                    "Cannot perform a binary operation between an unsigned tensor and a negative scalar. "
                    "Perform a explicit cast on one of them."
                )
            if ret_sca_ty.is_int():
                if lhs_is_scalar and not (
                    ret_sca_ty.get_int_min_value()
                    <= lhs_scalar
            ...
            lhs = (
                self.scalar_constant(lhs_scalar, dtype=ret_sca_ty)
                if lhs_is_scalar
                else self.cast(lhs, ret_sca_ty)
            )
            rhs = (
                self.scalar_constant(rhs_scalar, dtype=ret_sca_ty)
                if rhs_is_scalar
                else self.cast(rhs, ret_sca_ty)
            )
    ...
    def binary_op_sanitize_overflow_impl(
        self, lhs: TensorTy, rhs: TensorTy, binary_op: callable
    ):
        if (
            lhs.type.scalar.int_bitwidth >= 64
            or not self.builder.options.sanitize_overflow
        ):
            return
        lhs_sca_ty = lhs.type.scalar
        rhs_sca_ty = rhs.type.scalar
        ...
        cond = self.and_(
            self.less_equal(ret, max_value), self.greater_equal(ret, min_value)
        )
    ...
    def add(
        ...
        if input_scalar_ty.is_ptr():
            ...
            return self.tensor(
                self.builder.create_addptr(input.handle, other_handle), input.type
            )
        # float + float
        elif input_scalar_ty.is_floating():
            return self.tensor(
                self.builder.create_fadd(input.handle, other.handle), input.type
            )
        # int + int
            ...
            return self.tensor(
                self.builder.create_add(input.handle, other.handle), input.type
            )
    ...
    def sub(
        ...
        if scalar_ty.is_floating():
            return self.tensor(
                self.builder.create_fsub(input.handle, other.handle), input.type
            )
        # int - int
            ...
            return self.tensor(
                self.builder.create_sub(input.handle, other.handle), input.type
            )
    ...
    def mul(
        ...
        if scalar_ty.is_floating():
            return self.tensor(
                self.builder.create_fmul(input.handle, other.handle), input.type
            )
        # int * int
        elif scalar_ty.is_int():
            if sanitize_overflow:
            ...
            return self.tensor(
                self.builder.create_mul(input.handle, other.handle), input.type
            )
        raise TypeError(f"unexpected type {scalar_ty}")

    def truediv(
        self, input: TensorTy | numbers.Number, other: TensorTy | numbers.Number
    ) -> TensorTy:
    ...
    def floordiv(
        ...
        if input_scalar_ty.is_int() and other_scalar_ty.is_int():
            ...
            if ret_ty.is_int_signed():
                return self.tensor(
                    self.builder.create_sdiv(input.handle, other.handle), input.type
                )
            else:
                return self.tensor(
                    self.builder.create_udiv(input.handle, other.handle), input.type
                )
        raise TypeError(f"unexpected type {input_scalar_ty}")
    ...
    def mod(
        ...
        if scalar_ty.is_floating():
            return self.tensor(
                self.builder.create_frem(input.handle, other.handle), input.type
            )
        # % int
        elif scalar_ty.is_int():
            ...
            if scalar_ty.is_int_signed():
                return self.tensor(
                    self.builder.create_srem(input.handle, other.handle), input.type
                )
            else:
                return self.tensor(
                    self.builder.create_urem(input.handle, other.handle), input.type
                )
        raise TypeError(f"unexpected type {scalar_ty}")
    ...
    def clamp(
        self, x: TensorTy, min: TensorTy, max: TensorTy, propagate_nan: tl.PropagateNan
    ):
        ...
        if dtype.is_floating():
            return self.tensor(
                self.builder.create_clampf(
                    x.handle, min.handle, max.handle, propagate_nan
                ),
                x.type,
            )
        else:
    ...
    def bitwise_op_type_checking_impl(
        self, input: TensorTy, other: TensorTy
    ) -> Tuple[TensorTy, TensorTy]:
        input, other = self.binary_op_type_checking_impl(input, other)
    ...
    def and_(self, input: TensorTy, other: TensorTy) -> TensorTy:
        ...
        return self.tensor(
            self.builder.create_and(input.handle, other.handle), input.type
        )

    def or_(self, input: TensorTy, other: TensorTy) -> TensorTy:
        input, other = self.bitwise_op_type_checking_impl(input, other)
        return self.tensor(
            self.builder.create_or(input.handle, other.handle), input.type
        )

    def xor_(self, input: TensorTy, other: TensorTy) -> TensorTy:
        input, other = self.bitwise_op_type_checking_impl(input, other)
        return self.tensor(
            self.builder.create_xor(input.handle, other.handle), input.type
        )
    ...
    def shl(self, input: TensorTy, other: TensorTy) -> TensorTy:
        input, other = self.bitwise_op_type_checking_impl(input, other)
        return self.tensor(
            self.builder.create_shl(input.handle, other.handle), input.type
        )
    ...
    def minus(self, input: TensorTy) -> TensorTy:
        ...
        _0 = self.tensor(
            self.builder.get_null_value(input_sca_ty.to_ir(self.builder)), input_sca_ty
        )
    ...
    def reshape(
        self, input: TensorTy, dst_shape: List[int], can_reorder: bool
    ) -> TensorTy:
        numel = 1
        for s in dst_shape:
            numel *= s
        ...
        return self.tensor(
            self.builder.create_reshape(input.handle, dst_shape, can_reorder), ret_ty
        )
    ...
    def broadcast_impl_value(self, lhs: TensorTy, rhs: TensorTy) -> TensorTy:
        ...
        if lhs_ty.is_block() and not rhs_ty.is_block():
            ...
            rhs = self.tensor(
                self.builder.create_splat(rhs_ty.to_ir(self.builder), rhs.handle),
                rhs_ty,
            )
        # make_shape_compatible(scalar, block)
            ...
            lhs = self.tensor(
                self.builder.create_splat(lhs_ty.to_ir(self.builder), lhs.handle),
                lhs_ty,
            )
        # make_shape_compatible(block, block)
            ...
            if lhs_shape != ret_shape:
                ...
                lhs = self.tensor(
                    self.builder.create_broadcast(lhs.handle, ret_shape), ret_ty
                )
            if rhs_shape != ret_shape:
                ret_ty = tl.block_type(rhs_ty.scalar, ret_shape)
                rhs = self.tensor(
                    self.builder.create_broadcast(rhs.handle, ret_shape), ret_ty
                )
    ...
    def bitcast(self, input: TensorTy, dst_ty: tl.dtype) -> TensorTy:
        ...
        return self.tensor(
            self.builder.create_bitcast(input.handle, dst_ty.to_ir(self.builder)),
            dst_ty,
        )
    ...
    def cast(
        ...
        if (
            dst_sca_ty.is_floating()
            and src_sca_ty.is_floating()
            ...
            elif fp_downcast_rounding != ir.ROUNDING_MODE.RTNE:
            ...
            if fp_downcast_rounding is not None:
                raise ValueError(
                    ...
                    "Source scalar type is "
                    + str(src_sca_ty)
                    + " and destination type is "
                    + str(dst_sca_ty)
                )
        ...
        if (
            (src_sca_ty.is_fp8() and dst_sca_ty.is_floating())
            or (src_sca_ty.is_floating() and dst_sca_ty.is_fp8())
            or use_custom_rounding
        ):
            return self.tensor(
                self.builder.create_fp_to_fp(
                    input.handle, dst_ty.to_ir(self.builder), fp_downcast_rounding
                ),
                dst_ty,
            )
        ...
        if truncate_fp:
            return self.tensor(
                self.builder.create_fp_trunc(input.handle, dst_ty.to_ir(self.builder)),
                dst_ty,
            )
        ...
        if ext_fp:
            return self.tensor(
                self.builder.create_fp_ext(input.handle, dst_ty.to_ir(self.builder)),
                dst_ty,
            )
        ...
        if (
            ...
            if dst_sca_ty.is_bool():
                ...
                return self.tensor(
                    self.builder.create_int_cast(
                        input.handle, dst_ty.to_ir(self.builder), sign_extend
                    ),
                    dst_ty,
                )
        ...
        if src_sca_ty.is_standard_floating() and dst_sca_ty.is_int():
            if dst_sca_ty.is_bool():
                ...
                return self.tensor(
                    self.builder.create_fp_to_si(
                        input.handle, dst_ty.to_ir(self.builder)
                    ),
                    dst_ty,
                )
            else:
                return self.tensor(
                    self.builder.create_fp_to_ui(
                        input.handle, dst_ty.to_ir(self.builder)
                    ),
                    dst_ty,
                )
        ...
        if src_sca_ty.is_ptr() and dst_sca_ty.is_int():
            ...
            if bitwidth == 64:
                return self.tensor(
                    self.builder.create_ptr_to_int(
                        input.handle, dst_ty.to_ir(self.builder)
                    ),
                    dst_ty,
                )
        ...
        if src_sca_ty.is_int() and dst_sca_ty.is_ptr():
            return self.tensor(
                self.builder.create_int_to_ptr(
                    input.handle, dst_ty.to_ir(self.builder)
                ),
                dst_ty,
            )
    ...
    def _str_to_padding_option(self, padding_option):
        ...
        if padding_option:
            ...
            elif padding_option == "neg_inf":
                padding = ir.PADDING_OPTION.PAD_NEG_INF
            elif padding_option == "inf":
                padding = ir.PADDING_OPTION.PAD_INF
            else:
    ...
    def _canonicalize_boundary_check(self, boundary_check, block_shape):
        if boundary_check:
            ...
            assert len(boundary_check) == len(set(boundary_check)), (
                "Duplicate dimension in `boundary_check`"
            )
            return sorted(boundary_check)
        return ()

    def _load_block_pointer(
        self, ptr, mask, other, boundary_check, padding, cache, eviction, is_volatile
    ):
        ...
        # Build IR
        return self.tensor(
            self.builder.create_tensor_pointer_load(
                ptr.handle, boundary_check, padding, cache, eviction, is_volatile
            ),
            dst_ty,
        )
    ...
    def _load_legacy(
        ...
        if mask is None:
            ret = self.tensor(
                self.builder.create_load(ptr.handle, cache, eviction, is_volatile),
                dst_ty,
            )
        else:
            ret = self.tensor(
    ...
    def load(
        ...
        if ptr.type.is_ptr() and ptr.type.element_ty.is_block():
            ...
            return self._load_block_pointer(
                ptr, mask, other, boundary_check, padding, cache, eviction, is_volatile
            )
        else:
            # Load by a tensor of pointers or a pointer of scalar: `block_type<pointer_type<>>` or `pointer_type<>`
    ...
    def validate_store_like(
        self, desc: tl.tensor_descriptor_base, value: TensorTy, offsets
    ) -> None:
        assert isinstance(desc, tl.tensor_descriptor_base)
        ndim = len(desc.block_shape)
        assert len(offsets) == ndim, f"expected {ndim} offsets, but got {len(offsets)}"
    ...
    def descriptor_atomic_add(
        ...
        return self.tensor(
            self.builder.create_descriptor_reduce(
                kind, desc.handle, value.handle, offsets
            ),
            tl.void,
        )
    ...
    def descriptor_atomic_and(
        self, desc: tl.tensor_descriptor_base, value: TensorTy, offsets
    ) -> TensorTy:
        self.validate_store_like(desc, value, offsets)
        assert desc.dtype in {tl.uint32, tl.int32, tl.uint64, tl.int64}, (
            "Unsupported dtype"
        )
        offsets = self._convert_to_ir_values(offsets, require_i64=False)
    ...
    def descriptor_gather(
        self, desc, x_offsets, y_offset, cache_modifier: str, eviction_policy: str
    ) -> TensorTy:
        assert isinstance(desc, tl.tensor_descriptor_base)
        ...
        # Validate descriptor.
        assert len(desc.block_shape) == 2, (
            f"descriptor must be 2D, but got {desc.block_shape}"
        )
        assert desc.block_shape[0] == 1, (
            f"descriptor block must have 1 row, but got {desc.block_shape}"
        )

        # Validate offsets.
        assert len(x_offsets.shape) == 1, (
            f"x offsets must be 1D, but got {x_offsets.shape}"
        )

        # Validate minimum block size.
        assert x_offsets.shape[0] >= 8, (
            f"descriptor gather must have at least 8 rows, but got {x_offsets.shape}"
        )
        dtype = desc.dtype
        ...
        x = self.builder.create_descriptor_gather(
            desc.handle, x_offsets.handle, y_offset, type.to_ir(self.builder)
        )
        return self.tensor(x, type)

    def descriptor_scatter(
        self, desc, value: TensorTy, x_offsets, y_offset
    ) -> TensorTy:
        assert isinstance(desc, tl.tensor_descriptor_base)
        ...
        # Validate offsets.
        assert len(x_offsets.shape) == 1, (
            f"x offsets must be 1D, but got {x_offsets.shapae}"
        )

        # Validate minimum block size.
        assert x_offsets.shape[0] >= 8, (
            f"descriptor scatter must have at least 8 rows, but got {x_offsets.shape}"
        )
        dtype = desc.dtype
        min_cols = 32 // dtype.primitive_bitwidth * 8
        assert desc.block_shape[1] >= min_cols, (
        ...
        y_offset = self._convert_to_ir_values((y_offset,), require_i64=False)[0]
        self.builder.create_descriptor_scatter(
            desc.handle, value.handle, x_offsets.handle, y_offset
        )
    ...
    def _store_block_pointer(self, ptr, val, mask, boundary_check, cache, eviction):
        ...
        assert ptr.type.element_ty.element_ty == val.type.element_ty, (
            f"Block element type({ptr.type.element_ty.element_ty}) and value element type({val.type.element_ty}) mismatch"
        )
        ...
        # Build IR
        return self.tensor(
            self.builder.create_tensor_pointer_store(
                ptr.handle, val.handle, boundary_check, cache, eviction
            ),
            tl.void,
        )
    ...
    def _store_legacy(self, ptr, val, mask, boundary_check, cache, eviction):
        ...
        if mask is None:
            return self.tensor(
                self.builder.create_store(ptr.handle, val.handle, cache, eviction),
                tl.void,
            )
        if not mask.type.scalar.is_bool():
            raise ValueError("Mask must have boolean scalar type")
        return self.tensor(
            self.builder.create_masked_store(
                ptr.handle, val.handle, mask.handle, cache, eviction
            ),
            tl.void,
        )
    ...
    def store(
        ...
        if ptr.type.is_ptr() and ptr.type.element_ty.is_block():
            # Store by a block pointer: `pointer_type<block_type<>>`
            return self._store_block_pointer(
                ptr, val, mask, boundary_check, cache, eviction
            )
        else:
            # Store by a tensor of pointers or a pointer of scalar: `block_type<pointer_type<>>` or `pointer_type<>`
    ...
    def atomic_cas(
        self, ptr: TensorTy, cmp: TensorTy, val: TensorTy, sem: str, scope: str
    ) -> TensorTy:
        ...
        return self.tensor(
            self.builder.create_atomic_cas(
                ptr.handle, cmp.handle, val.handle, sem, scope
            ),
            val.type,
        )
    ...
    def atomic_max(
        self, ptr: TensorTy, val: TensorTy, mask: TensorTy, sem: str, scope: str
    ) -> TensorTy:
    ...
    def atomic_add(
        ...
        return self.tensor(
            self.builder.create_atomic_rmw(
                op, ptr.handle, val.handle, mask.handle, sem, scope
            ),
            val.type,
        )
    ...
    def atomic_and(
        ...
        return self.tensor(
            self.builder.create_atomic_rmw(
                ir.ATOMIC_OP.AND, ptr.handle, val.handle, mask.handle, sem, scope
            ),
            val.type,
        )
    ...
    def atomic_or(
        ...
        return self.tensor(
            self.builder.create_atomic_rmw(
                ir.ATOMIC_OP.OR, ptr.handle, val.handle, mask.handle, sem, scope
            ),
            val.type,
        )
    ...
    def atomic_xor(
        ...
        return self.tensor(
            self.builder.create_atomic_rmw(
                ir.ATOMIC_OP.XOR, ptr.handle, val.handle, mask.handle, sem, scope
            ),
            val.type,
        )
    ...
    def atomic_xchg(
        ...
        return self.tensor(
            self.builder.create_atomic_rmw(
                ir.ATOMIC_OP.XCHG, ptr.handle, val.handle, mask.handle, sem, scope
            ),
            val.type,
        )
    ...
    def _str_to_dot_input_precision(self, input_precision):
        assert (
            input_precision.lower() in self.builder.options.allowed_dot_input_precisions
        ), (
    ...
    def _bitcast_to_fp_type(self, val: TensorTy, float_format: str):
        ...
        if triton_ty is None:
            assert float_format == "e2m1", (
                f"Internal Error: Unexpected float format: {float_format}"
            )
            assert val.dtype == tl.uint8, (
                f"e2m1 format must be packed as uint8. Got {val.dtype}"
            )
            return val
        if val.dtype == triton_ty:
            ...
            assert val.dtype == unsigned_ty, (
                f"Unexpected dtype for {float_format}. Got {val.dtype}"
            )
    ...
    def verify_scaled_shape(self, M, N, K, lhs_scale, rhs_scale):
        if lhs_scale is not None:
            ...
            assert lhs_scale_shape == [M, K // scale_factor], (
        ...
        if rhs_scale is not None:
            ...
            assert rhs_scale_shape == [N, K // scale_factor], (
    ...
    def dot_scaled(
        ...
        rhs_scale_is_none = rhs_scale is None or (
            isinstance(rhs_scale, tl.constexpr) and rhs_scale.value is None
        )
        lhs_scale_is_none = lhs_scale is None or (
            isinstance(lhs_scale, tl.constexpr) and lhs_scale.value is None
        )
        lhs = self._bitcast_to_fp_type(lhs, lhs_format)
        rhs = self._bitcast_to_fp_type(rhs, rhs_format)

        assert lhs_k_pack or lhs_format == "e2m1", (
            "only mxfp4 inputs can be packed along a dimension different than K"
        )
        assert rhs_k_pack or rhs_format == "e2m1", (
            "only mxfp4 inputs can be packed along a dimension different than K"
        )
        ...
        assert PACKED_B_DIM == PACKED_A_DIM, (
    ...
    def where(self, condition: TensorTy, x: TensorTy, y: TensorTy) -> TensorTy:
        ...
        ret_ty = x.type
        return self.tensor(
            self.builder.create_select(condition.handle, x.handle, y.handle), ret_ty
        )
    ...
    def reduction(
        self, inputs: Sequence[TensorTy], axis: int, region_builder_fn
    ) -> Tuple[TensorTy, ...]:
        if axis is None:
            inputs = tuple(
                self.reshape(t, [t.numel.value], can_reorder=True) for t in inputs
            )
            axis = 0
        # get result shape
        ...
        assert all(t.type.shape == shape for t in inputs), (
            "all reduction inputs must have the same shape"
        )
        ...
        return tuple(
            self.wrap_tensor(reduce_op.get_result(i), inputs[i].type.scalar, ret_shape)
    ...
    def gather(self, src: TensorTy, index: TensorTy, axis: int) -> TensorTy:
        ...
        assert len(index.type.shape) == rank, (
            "source and index tensors must have the same rank"
        )

        assert -rank <= axis < rank, (
            f"gather axis {axis} must be < source rank ({rank})"
        )
        if axis < 0:
            axis += rank
        ...
        for d in range(rank):
            if d == axis:
                continue
            assert index.type.shape[d] == src.type.shape[d], (
                f"index dim {axis} must match the corresponding source dim"
            )
    ...
    def map_elementwise(
        self,
        inputs: Sequence[tl.tensor],
        result_types: Sequence[tl.dtype],
        pack: int,
        region_builder_fn,
    ) -> Tuple[tl.tensor, ...]:
    ...
    def histogram(
        self, input: TensorTy, num_bins: int, mask: Optional[TensorTy]
    ) -> TensorTy:
    ...
    def multiple_of(self, x: TensorTy, values: List[int]) -> TensorTy:
        ...
        x.handle.set_attr(
            "tt.divisibility", ir.make_attr(values, x.handle.get_context())
        )
        return x
    ...
    def device_print(self, prefix: str, args: List[TensorTy], hex: bool) -> TensorTy:
        ...
        return self.tensor(
            self.builder.create_print(prefix, hex, new_args, is_signed), tl.void
        )

    def device_assert(
        self, cond: TensorTy, msg: str, mask: Optional[TensorTy]
    ) -> TensorTy:
        ...
        if isinstance(elem, tl.constexpr):
            ...
            if elem.dtype != tl.int64 and require_i64:
                return self.builder.create_int_cast(
                    elem.handle, self.builder.get_int64_ty(), elem.dtype.is_int_signed()
                )
            elif elem.dtype == tl.int64 and not require_i64:
                assert False, (
                    "Block pointers only support 32 bit `offsets/block_shape`, "
    ...
    def _convert_to_ir_values(self, list_like, require_i64=True):
        if hasattr(list_like, "__iter__"):
            return [
                self._convert_elem_to_ir_value(elem, require_i64) for elem in list_like
            ]
        return [self._convert_elem_to_ir_value(list_like, require_i64)]

    def make_block_ptr(
        self, base: TensorTy, shape, strides, offsets, block_shape, order
    ) -> TensorTy:
        ...
        assert all(
            isinstance(elem, int) and -(2**31) <= elem < 2**31 for elem in block_shape
        ...
        if not hasattr(order, "__iter__"):
            order = [order]
        order = [
            elem.value if isinstance(elem, tl.constexpr) else elem for elem in order
        ]
        ...
        assert all(
            len(block_shape) == len(list_like)
            for list_like in [shape, strides, offsets, order]
        ...
        handle = self.builder.create_make_block_ptr(
            base.handle, shape, strides, offsets, block_shape, order
        )
        return self.tensor(
            handle, tl.pointer_type(tl.block_type(base.type.element_ty, block_shape))
        )
    ...
    def make_tensor_descriptor(
        ...
        strides = [
            self.make_scalar(tl._unwrap_if_constexpr(x), tl.int64) for x in strides
        ]
