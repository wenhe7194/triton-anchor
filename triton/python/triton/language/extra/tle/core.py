...
def _linearize_dynamic_multidim_index(
    index_tuple, src_shape, tile_shape_ints, _semantic
):
    """
    ...
    if len(index_tuple) != len(grid):
        raise ValueError(
            f"Dynamic multi-dim index rank {len(index_tuple)} does not match grid rank {len(grid)}"
        )
    ...
    if isinstance(index, tl.tensor):
        ...
        if isinstance(index_unwrapped, (tuple, list, tl.tuple)):
            ...
            if has_tensor:
                ...
                # ====================================
                index_ir_handle = _linearize_dynamic_multidim_index(
                    index_unwrapped, src_shape, tile_shape_ints, _semantic
                )
                is_dynamic = True
            else:
                # ====================================
                ...
                for v in index_unwrapped:
                    ...
                    if iv is None:
                        raise ValueError(
                            "Multi-dim index must contain int/constexpr values."
                        )
                    idx_ints.append(iv)
    ...
    try:
        if is_dynamic:
            ...
            index_ir = _semantic._convert_to_ir_values(
                [index_value], require_i64=False
            )[0]

        output = _semantic.builder.create_extract_tile(
            x.handle, index_ir, tile_shape_ints
        )
...
def insert_tile(
    ...
    grid = []
    for i, (src_dim, tile_dim) in builtins.enumerate(
        builtins.zip(src_shape, tile_shape)
    ):
        if tile_dim <= 0:
    ...
    if isinstance(index, tl.tensor):
        ...
        if isinstance(index_unwrapped, (tuple, list, tl.tuple)):
            ...
            if has_tensor:
                index_ir_handle = _linearize_dynamic_multidim_index(
                    index_list, src_shape, tile_shape, _semantic
                )
                is_dynamic = True
            # ------------------------------------------------
