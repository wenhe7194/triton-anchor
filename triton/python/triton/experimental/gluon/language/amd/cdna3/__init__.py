...
_atomic_op_str_to_op = {
    "smax": ir.ATOMIC_OP.MAX,
    "smin": ir.ATOMIC_OP.MIN,
    "umax": ir.ATOMIC_OP.UMAX,
    "umin": ir.ATOMIC_OP.UMIN,
    "fadd": ir.ATOMIC_OP.FADD,
    "iadd": ir.ATOMIC_OP.ADD,
...
def _verify_buffer_ops(ptr, offsets, mask=None, other=None):
    ...
    assert isinstance(offsets.type, ttgl.distributed_type), (
        "expected offsets type to be a distributed_type"
    )
...
def _verify_element_type_and_dispatch_op(op, elem_type, arch):
    ...
    assert elem_type in supported_types, (
        f"{elem_type} is not supported in buffer atomic on {arch}."
    )

    if op in ["and", "or", "xor", "xchg"]:
        assert elem_type in [ttgl.int32, ttgl.int64], (
            f"{op} with {elem_type} is not supported on CDNA3 or CDNA4"
        )
    ...
    if op in ["max", "min"]:
        if elem_type in [ttgl.int32, ttgl.int64, ttgl.float64]:
            ...
            raise ValueError(
                f"{op} with {elem_type} is not supported on CDNA3 and CDNA4"
            )
    ...
    if op == "add":
        if elem_type in [ttgl.uint32, ttgl.uint64]:
            ...
            assert arch == "cdna4", (
                "Buffer atomic fadd with bf16 is only supported on CDNA4 for now."
            )
            op = "f" + op
            return _atomic_op_str_to_op[_unwrap_if_constexpr(op)]
        else:
...
def _buffer_atomic_rmw_impl(op, ptr, offsets, value, arch, mask, sem, scope, _semantic):
    ...
    return _semantic.tensor(
        _semantic.builder.create_buffer_atomic_rmw(
            op, ptr.handle, offsets.handle, value.handle, sem, scope, mask
        ),
        value.type,
    )
...
def buffer_load(ptr, offsets, mask=None, other=None, cache=None, _semantic=None):
    ...
    mask = mask.handle if mask is not None else ir.value()
    cache_modifier = (
        _semantic._str_to_load_cache_modifier(cache)
        if cache is not None
        else ir.CACHE_MODIFIER.NONE
    )

    ret_ty = offsets.type.with_element_ty(ptr.type.scalar.element_ty)
    builder = _semantic.builder
    handle = builder.create_buffer_load(
        ret_ty.to_ir(builder), ptr.handle, offsets.handle, mask, other, cache_modifier
    )
...
@builtin
def buffer_store(
    stored_value, ptr, offsets, mask=None, cache=None, _semantic: GluonSemantic = None
):
    """
    ...
    mask = mask.handle if mask is not None else ir.value()
    cache_modifier = (
        _semantic._str_to_store_cache_modifier(cache)
        if cache is not None
        else ir.CACHE_MODIFIER.NONE
    )

    _semantic.builder.create_buffer_store(
        stored_value.handle, ptr.handle, offsets.handle, mask, cache_modifier
    )
