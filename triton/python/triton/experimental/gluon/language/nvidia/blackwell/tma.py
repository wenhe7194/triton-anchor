...
def _check_gather_scatter(tensor_desc, x_offsets, smem, op_name, smem_name):
    ...
    assert len(tensor_desc.block_shape) == 2, (
        f"async {op_name} requires a 2D tensor descriptor, but got one with rank {len(tensor_desc.block_shape)}"
    )
    assert tensor_desc.layout == smem.layout, (
        f"tensor descriptor layout {tensor_desc.layout} does not match {smem_name} shared memory layout {smem.layout}"
    )
    # Row offsets must be 1D and have at least 8 rows.
    assert len(x_offsets.shape) == 1, (
        f"async {op_name} requires a 1D tensor of row offsets, but got one with rank {len(x_offsets.shape)}"
    )
    assert x_offsets.shape[0] >= 8, (
        f"async {op_name} requires at least 8 rows, but got {x_offsets.shape[0]}"
    )
    # Block shape must be [1, Y] where Y >= min_cols.
    min_cols = 32 // tensor_desc.dtype.primitive_bitwidth * 8
    assert tensor_desc.block_shape[0] == 1, (
        f"async {op_name} requires the tensor descriptor's block shape to have 1 row, but got {tensor_desc.block_shape}"
    )
    assert tensor_desc.block_shape[1] >= min_cols, (
        f"async {op_name} requires the tensor descriptor's block shape to have at least {min_cols} columns, but got {tensor_desc.block_shape[1]}"
    )


@builtin
def async_gather(
    tensor_desc, x_offsets, y_offset, barrier, result, pred=True, _semantic=None
):
    """
...
def async_scatter(tensor_desc, x_offsets, y_offset, src, _semantic=None):
    ...
    _semantic.builder.create_async_tma_scatter(
        tensor_desc.handle, x_offsets.handle, y_offset.handle, src.handle
    )
